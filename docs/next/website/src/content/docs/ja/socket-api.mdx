...
`layout.equalize` は、同じ方向に連なる分割へ幅または高さを均等に割り当てます。横分割と縦分割を組み合わせた構造は変えません。両方の id を省略するとアクティブなタブを、`tab_id` を渡すとそのタブを、`pane_id` を渡すとそのペインを含むタブを対象にします。レスポンスには、更新後の移植可能な `layout` と `type: "layout_equalized"` が含まれます。
