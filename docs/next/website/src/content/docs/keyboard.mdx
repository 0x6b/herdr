...
Press `prefix+?` at any time to see every active binding. Press `/` in the keybind help to filter actions and shortcuts, use Backspace to edit the filter or `ctrl+u` to clear it, then press Enter to run the selected command.

To reset resized panes, search for `equalize split sizes` and press Enter. Herdr gives panes in each horizontal or vertical run an even share of that axis without changing the mixed split structure. You can also bind the optional `keys.equalize_splits` action to a shortcut.
...
| Copy mode | `prefix+[` |
| Copy last command and output | `prefix+y` |

`prefix+y` copies the prompt, command line, and output. Press it repeatedly to cycle through older command blocks. The pane's shell must emit OSC 133 semantic prompt markers; without shell integration, the action does nothing.

Ghostty with Fish works automatically: Ghostty prepends its shell integration directory to `XDG_DATA_DIRS`, Herdr passes that environment to the pane, and Fish loads the integration from `vendor_conf.d` when it starts. For other shell and terminal combinations, make sure the shell launched inside the Herdr pane emits OSC 133 markers. In particular, Ghostty's automatic Bash and Zsh injection applies to the shell Ghostty launches directly; follow [Ghostty's manual shell integration setup](https://ghostty.org/docs/features/shell-integration#manual-shell-integration-setup) when the integration is not available inside a Herdr pane.
