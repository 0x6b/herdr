...
`layout.equalize` 会在水平或垂直方向连续分割的窗格之间均分宽度或高度,但不会改变水平分割与垂直分割混合后的结构。同时省略两个 id 时以活动标签页为目标;传入 `tab_id` 时以该标签页为目标;传入 `pane_id` 时以包含该窗格的标签页为目标。响应为 `type: "layout_equalized"`,并包含更新后的可移植 `layout`。
