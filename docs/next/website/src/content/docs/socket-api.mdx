...
`layout.equalize` gives panes in each horizontal or vertical split run an even
share of that axis without changing the mixed split structure. Target the
active tab by omitting both ids, a specific tab with `tab_id`, or the tab that
contains a pane with `pane_id`. The response is `type: "layout_equalized"` with
the updated portable `layout`.
