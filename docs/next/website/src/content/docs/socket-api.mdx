...
`layout.equalize` balances every run of same-direction splits. Omit both target
fields for the active tab, or pass either `tab_id` or `pane_id`. The response is
`type: "layout_equalized"` with the updated portable `layout`.

```json
{"id":"req_equalize","method":"layout.equalize","params":{"tab_id":"w1:t1"}}
```
