...
impl Terminal {
    ...
    ///
    /// Like Ghostty's command-output cycling action, this relies on OSC 133
    /// semantic prompt markers, skips the newest (current) prompt, and treats
    /// `n = 0` as the most recently completed command.
...
mod tests {
    ...
    #[test]
    fn command_output_includes_a_visible_prompt_for_an_empty_command() {
        let mut terminal = Terminal::new(20, 8, 4096).unwrap();
        terminal.write(
            b"\x1b]133;A\x07$ \x1b]133;B\x07first\r\n\x1b]133;C\x07one\r\n\x1b]133;D;0\x07\
    ...
    #[test]
    fn command_output_removes_leading_and_trailing_blank_lines() {
        let mut terminal = Terminal::new(20, 8, 4096).unwrap();
        terminal.write(
            b"\x1b]133;A\x07\r\n  \r\n$ \x1b]133;B\x07first\r\n\x1b]133;C\x07one\r\n\r\n  \r\n\x1b]133;D;0\x07\
              \x1b]133;A\x07$ \x1b]133;B\x07",
    ...
    #[test]
    fn command_output_requires_semantic_prompt_history() {
        let mut terminal = Terminal::new(20, 3, 4096).unwrap();
        terminal.write(b"$ echo one\r\none\r\n$ ");
